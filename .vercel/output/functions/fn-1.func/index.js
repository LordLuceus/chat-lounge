...
var init__ = __esm({
  ".svelte-kit/output/server/nodes/0.js"() {
    ...
    imports = ["_app/immutable/nodes/0.CvZhLGel.js", "_app/immutable/chunks/scheduler.CYFynt7j.js", "_app/immutable/chunks/index.iosNfj23.js"];
    stylesheets = [];
    fonts = [];
  }
});
          ...
          for (let attempts = 0; attempts < this.maxRetries; attempts++) {
            try {
              ...
              if (response.ok) {
                ...
                await new Promise(
                  (resolve2) => setTimeout(resolve2, Math.pow(2, attempts + 1) * 500)
                );
              } else {
          ...
          for await (const chunk of response) {
            ...
            while ((firstNewline = buffer.indexOf("\n")) !== -1) {
              ...
              if (chunkLine.startsWith("data:")) {
                const json2 = chunkLine.substring(6).trim();
                if (json2 !== "[DONE]") {
                  yield JSON.parse(json2);
                }
              }
            }
          }
...
function createParser(onParse) {
  ...
  function feed(chunk) {
    ...
    while (position < length) {
      ...
      let character;
      for (let index3 = startingPosition; lineLength < 0 && index3 < length; ++index3) {
        character = buffer[index3];
        if (character === ":" && fieldLength < 0) {
          fieldLength = index3 - position;
        } else if (character === "\r") {
          discardTrailingNewline = true;
          lineLength = index3 - position;
        } else if (character === "\n") {
          lineLength = index3 - position;
        }
      }
  ...
  function parseEventStreamLine(lineBuffer, index3, fieldLength, lineLength) {
    if (lineLength === 0) {
      if (data.length > 0) {
        onParse({
          type: "event",
    ...
    const field = lineBuffer.slice(index3, index3 + (noValue ? lineLength : fieldLength));
    let step = 0;
    if (noValue) {
      step = lineLength;
    } else if (lineBuffer[index3 + fieldLength + 1] === " ") {
      step = fieldLength + 2;
    } else {
      step = fieldLength + 1;
    }
    const position = index3 + step;
...
function hasBom(buffer) {
  return BOM.every((charCode, index3) => buffer.charCodeAt(index3) === charCode);
}
var BOM;
var init_dist = __esm({
  "node_modules/eventsource-parser/dist/index.js"() {
        ...
        choices: chunk.choices.map((choice) => {
          ...
          return {
            delta: {
              ...
              tool_calls: ((_e = (_d = choice.delta) == null ? void 0 : _d.toolCalls) == null ? void 0 : _e.length) ? (_g = (_f = choice.delta) == null ? void 0 : _f.toolCalls) == null ? void 0 : _g.map((toolCall, index3) => ({
                index: index3,
                id: toolCall.id,
...
function chunkToText() {
  ...
  return (json2) => {
    ...
    if (isChatCompletionChunk(json2)) {
      const delta = (_a = json2.choices[0]) == null ? void 0 : _a.delta;
...
var textStreamPart, functionCallStreamPart, dataStreamPart, errorStreamPart, assistantMessageStreamPart, assistantControlDataStreamPart, dataMessageStreamPart, toolCallStreamPart, messageAnnotationsStreamPart, streamParts, streamPartsByCode, validCodes, parseStreamPart, COMPLEX_HEADER, __internal__OpenAIFnMessagesSymbol, StreamingTextResponse, MISTRAL_API_KEY, config, mistral, POST;
var init_server_ts = __esm({
  ".svelte-kit/output/server/entries/endpoints/api/chat/_server.ts.js"() {
    ...
    MISTRAL_API_KEY = "ke5dOdnlAXkPX1jvjNwBNX0RwbZSaG0q";
    config = { runtime: "edge" };
    mistral = new client_default(MISTRAL_API_KEY);
    POST = async ({ request }) => {
      const { messages } = await request.json();
      const response = await mistral.chatStream({ model: "mistral-medium", messages });
