...
export async function createConversation({
  ...
  const conversation = (
    await db.insert(conversations).values({ name, agentId, modelId }).returning()
  ).at(0);
...
export async function updateConversation(
  ...
  return db.update(conversations).set(data).where(eq(conversations.id, conversationId)).returning();
}
