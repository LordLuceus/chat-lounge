...
export const POST = (async ({ locals, request }) => {
  ...
  const { messages, modelId, agentId, conversationId, regenerate } = await request.json();
  ...
  return aiService.getResponse(messages, model.id, userId, agent, conversationId, regenerate);
